...
// #include "../../include/testparsing.h"
//파싱이 완료되기전에 test하기 위해 만들었습니다.

t_pcon *miniparse(char *input)//이함수들을 가지고 만드시면 편하실 거에요!!
{
    t_pcon *head = malloc(sizeof(t_pcon));
    init_pcon_pan(head, NULL, LINKED_PCON);
    char **two = ft_split(input, " ");
    int idx = 0;
    while (two[idx])
    {
        insert_pan(head, new_pan(two[idx]));
        idx++;
    }
    remove_pan(head, search_pan(head, "|"));
    ft_free_two(two);
    print_pcon(head);
    return (head);
}//직접적인 test를 할 수 있는 함수입니다.
...
{
    t_pan *current;
    const char *type[] = {"T_REDIRECTION", "T_ARGV,", "T_ENV,", "T_NULL"};
    current = head->head;
    while (current)
    {
        printf("val : %s\n", current->val);
